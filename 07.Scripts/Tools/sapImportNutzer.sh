...
then
    ...
    result=$(curl -X POST -H "Content-Type:text/comma-separated-values; charset=UTF-8" -H "X-Auth:$token" --data-binary "@$3" http://localhost:8080/macoco-be/api/domain/csv/importnutzer)
    echo $result
    echo "Done"
fi