...
export class JsonApiService {
  ...
  /**
   * Deserialize json to GeneralError
   * @param json {any}
   * @return {GeneralError}
   */
  public static deserializeError(json: any, logger: Logger): GeneralError {
    let obj = GeneralError.create('MAF0xFFFF', 'Error', 'could not deserialize the error message');

    if (json !== undefined) {
      try {
        obj = GeneralError.fromJson(ApiService.deserialize(json, GeneralError));
      } catch (e) {
        logger.error('MAF0x008C: deserialize', {error: e, json: json, type: GeneralError});
      }
    }
