...
export class RidgelineChartComponent implements OnInit {
  ...
  ngAfterViewInit() {
    ...
    {
      this.plot_ridge(all_data[i], x_from, y_from + offset * i, width, height, x_range, y_range, x_start_value, new paper.Color(Math.random(), Math.random(), Math.random(), 0.2)); //TODO: Improve color (should be distinct, not white, ...)
    }
